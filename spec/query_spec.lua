...
describe('llquery', function()
  ...
  it("should merge duplicate keys into array", function()
    local tbl = URL.parse_query("k=1&k=2&k=3", {merge_duplicates = true})
    -- print("tbl:", inspect(tbl))
  ...
  it("should preserve original key case by default", function()
    local tbl = URL.parse_query("UserName=Tom&Role=Admin")
    -- print("tbl:", inspect(tbl))
    assert(tbl.UserName == "Tom", "UserName should keep original case")
    assert(tbl.Role == "Admin", "Role should keep original case")
    assert(tbl.username == nil, "lowercase username should not exist")
    assert(tbl.role == nil, "lowercase role should not exist")
  end)

  it("should lowercase keys when option enabled", function()
    local tbl = URL.parse_query("UserName=Tom&Role=Admin", {lowercase_keys = true})
    -- print("tbl:", inspect(tbl))
    assert(tbl.username == "Tom", "UserName should become lowercase")
    assert(tbl.role == "Admin", "Role should become lowercase")
    assert(tbl.UserName == nil, "original case should not exist")
  end)

  it("should trim values and lowercase keys if flags enabled", function()
    local tbl = URL.parse_query("A= 1 &B=2 ", {lowercase_keys = true, trim_values = true})
    -- print("tbl:", inspect(tbl))
    assert(type(tbl) == "table")
